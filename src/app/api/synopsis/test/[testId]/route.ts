...
import { checkTestSynopsisAccess, openSynopsis } from '@/server/services/synopsis-service';

/**
 * GET /api/synopsis/test/[testId] — streams one test's analysis PDF.
 *
...
export async function GET(_request: Request, ctx: { params: Promise<{ testId: string }> }) {
  ...
  if (access.state !== 'AVAILABLE') {
    const message =
      access.state === 'NOT_PUBLISHED'
        ? 'No analysis has been published for this test yet.'
